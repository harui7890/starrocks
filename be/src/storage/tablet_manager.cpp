...
TabletSharedPtr TabletManager::get_tablet(TTabletId tablet_id, bool include_deleted, std::string* err,int64_t staros_shardid) {
...
TabletSharedPtr TabletManager::_get_tablet_unlocked(TTabletId tablet_id, bool include_deleted, std::string* err,int64_t staros_shardid) {
...
TabletSharedPtr TabletManager::find_best_tablet_to_compaction(CompactionType compaction_type, DataDir* data_dir) {
    ...
    for (auto& tablets_shard : _tablets_shards) {
        ...
        for(auto tabletid:id_shard_set){
            ...
            {
                std::shared_lock rlock(tablets_shard->lock);
                auto st = _get_from_metacache(tabletid);
                if(!st.ok()){
                    rlock.unlock();
...
StatusOr<TabletSharedPtr> TabletManager::load_tablet_from_meta(DataDir* data_dir, TTabletId tablet_id, TSchemaHash schema_hash,
...
Status TabletManager::report_all_tablets_info(std::map<TTabletId, TTablet>* tablets_info) {
    ...
    for (auto& tablets_shard : _tablets_shards) {
        ...
        for(auto tablet_id:id_shard_set){
            ...
            {
                std::shared_lock rlock(tablets_shard->lock);
                auto  st= _get_from_metacache(tablet_id);
                if(!st.ok()){
                    rlock.unlock();
...
TabletSharedPtr TabletManager::_get_tablet_unlocked(TTabletId tablet_id, bool need_load,int64_t staros_shardid) {
    auto it = _get_from_metacache(tablet_id);
    if(it.status().ok()){
        return it.value();
    }
    if(need_load){
        ...
        _put_into_metacache(tablet_id, st.value());
        return st.value();
    }
    return nullptr;
}
...
// 在当前实现中，以下三个接口都依靠上层的shard lock保护，来保证原子性。尤其是put ifabsent中的get和put两个操作。
